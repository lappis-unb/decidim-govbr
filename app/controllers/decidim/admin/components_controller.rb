    ...
    class ComponentsController < Decidim::Admin::ApplicationController
      ...
      def create
        ...
        CreateComponent.call(@form) do
          on(:ok) do
            flash[:notice] = I18n.t("components.create.success", scope: DECIDIM_ADMIN_SCOPE)
            redirect_to action: :index
          end

          on(:invalid) do
            flash.now[:alert] = I18n.t("components.create.error", scope: DECIDIM_ADMIN_SCOPE)
            render action: "new"
          end
        end
      end
      ...
      def update
        ...
        UpdateComponent.call(@form, @component, current_user, current_participatory_space) do
          on(:ok) do |settings_changed, previous_settings, current_settings|
            ...
            flash[:notice] = I18n.t("components.update.success", scope: DECIDIM_ADMIN_SCOPE)
            redirect_to action: :index
          end

          on(:invalid) do
            flash[:alert] = I18n.t("components.update.error", scope: DECIDIM_ADMIN_SCOPE)
            render action: :edit
          end
        end
      end
      ...
      def destroy
        ...
        DestroyComponent.call(@component, current_user) do
          on(:ok) do
            flash[:notice] = I18n.t("components.destroy.success", scope: DECIDIM_ADMIN_SCOPE)
            redirect_to action: :index
          end

          on(:invalid) do
            flash[:alert] = I18n.t("components.destroy.error", scope: DECIDIM_ADMIN_SCOPE)
            redirect_to action: :index
          end
        end
      end
      ...
      def publish
        ...
        PublishComponent.call(@component, current_user) do
          on(:ok) do
            flash[:notice] = I18n.t("components.publish.success", scope: DECIDIM_ADMIN_SCOPE)
            redirect_to action: :index
          end
        end
      end
      ...
      def unpublish
        ...
        UnpublishComponent.call(@component, current_user) do
          on(:ok) do
            flash[:notice] = I18n.t("components.unpublish.success", scope: DECIDIM_ADMIN_SCOPE)
            redirect_to action: :index
          end
        end
      end
