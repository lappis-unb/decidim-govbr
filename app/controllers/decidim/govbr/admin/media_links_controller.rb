      ...
      class MediaLinksController < Decidim::Admin::ApplicationController
        ...
        def create
          ...
          CreateMediaLink.call(@form, current_user, current_participatory_space) do
            on(:ok) do
              flash[:notice] = I18n.t("media_links.create.success", scope: SCOPE)
              redirect_to participatory_space_media_links_path(current_participatory_space)
            end

            on(:invalid) do
              flash[:alert] = I18n.t("media_links.create.error", scope: SCOPE)
              render :new
            end
          end
        end
        ...
        def update
          ...
          UpdateMediaLink.call(@form, @media_link) do
            on(:ok) do
              flash[:notice] = I18n.t("media_links.update.success", scope: SCOPE)
              redirect_to participatory_space_media_links_path(current_participatory_space)
            end

            on(:invalid) do
              flash.now[:alert] = I18n.t("media_links.update.error", scope: SCOPE)
              render :edit
            end
          end
        end
        ...
        def destroy
          ...
          DestroyMediaLink.call(@media_link, current_user) do
            on(:ok) do
              flash[:notice] = I18n.t("media_links.destroy.success", scope: SCOPE)
              redirect_to participatory_space_media_links_path(current_participatory_space)
            end
          end
        end
