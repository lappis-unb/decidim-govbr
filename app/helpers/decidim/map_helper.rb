      ...
      if map_utility_static
        ...
        if map_utility_static.url(latitude: resource.latitude, longitude: resource.longitude)
          return link_to map_url, target: "_blank", rel: "noopener" do
            image_tag 'images/map.png', size: 120, alt: "#{map_service_brand} - #{address_text}"
          end
        end
      end

      ## begin GOVBR
      return link_to t("map_link_hint", address: address_text, scope: "decidim.map"), map_url, target: "_blank", rel: "noopener"
      ## end GOVBR
