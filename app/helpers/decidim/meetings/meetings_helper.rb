  ...
  module Meetings
    ...
    module MeetingsHelper
      ...
      def display_duration_agenda_items(agenda_item_id, index, agenda_items_times)
        html = ""
        text = "#{agenda_items_times[index][:start_time].strftime("%H:%M")} às #{agenda_items_times[index][:end_time].strftime("%H:%M")}"
        html += text if agenda_item_id == agenda_items_times[index][:agenda_item_id]
        html.html_safe
      end
