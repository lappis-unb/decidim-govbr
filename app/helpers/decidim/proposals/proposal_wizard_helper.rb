      ...
      def wizard_steps(current_step)
        ...
        content_tag(:ol, id: "wizard-steps", class: "wizard-steps") do
          proposal_wizard_steps.map do |wizard_step|
            ...
            attributes = active ? { "aria-current": "step", "aria-label": "#{t("current_step", scope:"wizard_steps")}: #{t(wizard_step, scope:"proposals")}", data: { active: "" } } : inactive_data
            content_tag(:li, t(wizard_step, scope:"proposals"), attributes)
          end.join.html_safe
        end
      end
