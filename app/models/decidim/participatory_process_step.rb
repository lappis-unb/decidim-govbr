...
module Decidim
  ...
  class ParticipatoryProcessStep < ApplicationRecord
    ...
    after_create :enqueue_change_active_step_job
    after_update :clear_previous_step_jobs, if: :saved_change_to_start_date?
    after_update :enqueue_change_active_step_job, if: :saved_change_to_start_date?
    ...
    def enqueue_change_active_step_job
      return unless start_date.present?

      return ParticipatoryProcesses::ChangeActiveStepJob.set(wait_until: start_date).perform_later(start_date.to_s) if start_date.future?

      ParticipatoryProcesses::ChangeActiveStepJob.perform_later(start_date.to_s)
    end

    def clear_previous_step_jobs
      job_class = ParticipatoryProcesses::ChangeActiveStepJob
      job_class.clear(start_date: start_date_before_last_save.to_s)
    end
  end
end
