    ...
    context "when scoped resource is present and order is valid" do
      ...
      it "only affects to content blocks associated with the resource" do
        ...
        order.each_with_index do |id, index|
          ...
          expect(Decidim::ContentBlock.for_scope(scope, organization: organization).where(scoped_resource_id: scoped_resource_id, id: id)).to exist
          expect(Decidim::ContentBlock.for_scope(scope, organization: organization).find_by(scoped_resource_id: scoped_resource_id, id: id).weight).to eq index + 1
        end
      end
    end
  end
