...
describe Decidim::Admin::Permissions do
  ...
  let(:user) { build :user, :admin, email: user_email, organization: organization }
  let(:organization) { build :organization, super_admins: super_admins }
  let(:super_admins) { [] }
  let(:context) { {} }
